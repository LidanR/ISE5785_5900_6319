...
class TubeTests {
    // Ray along the X-axis starting at the origin
    private final Ray r1 = new Ray(Point.ZERO, new Vector(1, 0, 0));
    // Tube with radius 1 and axis along the Z-axis
    private final Tube tube = new Tube(1,new Ray(Point.ZERO, new Vector(0, 0, 1)));
    /**
    ...
    void testGetNormal() {
        ...
        assertDoesNotThrow(() -> new Tube( 1,r1).getNormal(new Point(0, 1, 0)), "Failed to throw an exception when the point is vertical to the axis");
    }
    ...
    /**
     * Test method for {@link geometries.Tube#calculateIntersections(Ray, double)}.
     */
    @Test
    void testIntersectionWithDistance() {
        ...
        assertNull(tube.calculateIntersections(new Ray(new Point(3, 0, 0.5), new Vector(-1, 0, 0)), 1),
        ...
        assertEquals(2, tube.calculateIntersections(new Ray(new Point(3, 0, 0.5), new Vector(-1, 0, 0)), 10).size(),
